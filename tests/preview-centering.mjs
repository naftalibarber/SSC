...
assert.equal(Number(net.dataset.previewStickerDevicePixels),16);
assert.equal(Number(net.dataset.previewSeparatorDevicePixels),1);
assert.equal(net.style.getPropertyValue('--ssc-preview-step'),'51px');
assert.equal(net.style.getPropertyValue('--ssc-preview-net-gap'),'0px');
assert.equal(fallbackWidth,205);
assert.equal(fallbackHeight,154);
