...
const trainingCss=fs.readFileSync('code/css/training-upgrades.css','utf8');
...
const compactTrainingCss=trainingCss.replace(/\s+/g,'');
...
assert.match(compactTrainingCss,/\.full-history-list\{[^}]*overflow-y:auto/,'The complete history must scroll inside its dialog.');
assert.match(compactTrainingCss,/\.full-history-list\.full-history-row\{[^}]*grid-template-columns:44pxminmax\(0,1fr\)!important/,'Full-history rows must reserve a stable index column.');
...
assert.match(index,/code\/css\/training-upgrades\.css\?v=20260831-full-history-1/,'The full-history dialog styles must bypass cached CSS.');
assert.match(index,/code\/js\/app\.js\?v=20260831-full-history-1/,'The full-history behavior must bypass cached JavaScript.');
...
assert.match(workflow,/code\/css\/training-upgrades\.css/,'CI must run when the full-history stylesheet changes.');
