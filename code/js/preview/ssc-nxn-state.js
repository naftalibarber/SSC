(() => {
  ...
  function makeSticker(face,row,col,n,coords,outer){
    ...
    return{position,normal:[...FACE_NORMALS[face]],identity:face,solvedFace:face,solvedRow:row,solvedCol:col};
  }
  ...
  function solvedStickers(order){
    ...
    const outer=n-1;
    const stickers=[];
    for(const face of FACE_ORDER)for(let row=0;row<n;row++)for(let col=0;col<n;col++)stickers.push(makeSticker(face,row,col,n,coords,outer));
    return stickers;
  }
  ...
  function rotateVector(vector,axis,quarterDirection){
    ...
    return[
      Math.round((crossProduct[0]*quarterDirection)+(axis[0]*projection)),
      Math.round((crossProduct[1]*quarterDirection)+(axis[1]*projection)),
      Math.round((crossProduct[2]*quarterDirection)+(axis[2]*projection))
    ];
  }
  ...
  function parseMove(move,order){
    ...
    // Real 3BLD scrambles generated by cubing.js are oriented and may contain
    // legal 3x3 wide turns (for example Fw', Rw or Uw2). Two-layer wide turns
    // are valid from 3x3 upward; only 2x2 keeps them disabled.
    if(layerDepth>1&&n<3)return null;
    return Object.freeze({token,face,layerDepth,halfTurn:suffix.startsWith('2'),prime:suffix.includes("'")});
  }
  ...
  function applyMove(stickers,move,order,{strict=false}={}){
    ...
    if(!parsed){if(strict)throw new Error(`Unsupported ${order}x${order} move: ${String(move)}`);return false;}
  ...
  function faceFromNormal(normal){
    const [x,y,z]=normal;
    if(y===1)return'U';if(y===-1)return'D';if(z===1)return'F';if(z===-1)return'B';if(x===1)return'R';if(x===-1)return'L';
    throw new Error(`Invalid sticker normal: ${normal.join(',')}`);
  }

  function stickerCell(sticker,face,order,coords){
    const n=order,indexOf=value=>coords.indexOf(value),[x,y,z]=sticker.position;
  ...
  function stateToFaces(stickers,order){
    const n=assertOrder(order),coords=coordsForSize(n);
  ...
  function stickerId(face,row,col,order){return`${face}${(row*order)+col+1}`;}
  function buildStickerMapping(faces,order){
    const n=assertOrder(order),mapping={};
    for(const face of FACE_ORDER)for(let row=0;row<n;row++)for(let col=0;col<n;col++)mapping[stickerId(face,row,col,n)]=faces[face][row][col];
    return Object.freeze(mapping);
  }

  function validateFaces(faces,order){
    const n=assertOrder(order),counts=Object.fromEntries(FACE_ORDER.map(face=>[face,0]));
    for(const face of FACE_ORDER){
      ...
      for(const row of faces[face]){
        ...
        for(const identity of row){if(!FACE_IDENTITIES.has(identity))throw new Error(`Invalid face identity ${identity} in ${face}.`);counts[identity]+=1;}
      }
    }
    const expected=n*n;
    for(const face of FACE_ORDER)if(counts[face]!==expected)throw new Error(`Invalid ${face} sticker count: ${counts[face]} instead of ${expected}.`);
    return true;
  }

  function buildState(scramble,order,{strict=false}={}){
    const n=assertOrder(order),moves=normalizeScramble(scramble),stickers=solvedStickers(n),ignoredMoves=[];
    for(const move of moves)if(!applyMove(stickers,move,n,{strict}))ignoredMoves.push(move);
    const faces=stateToFaces(stickers,n);validateFaces(faces,n);
    return Object.freeze({order:n,moves:Object.freeze([...moves]),ignoredMoves:Object.freeze(ignoredMoves),faces,mapping:buildStickerMapping(faces,n),engineState:stickers});
  }

  function inverseMove(move,order){const parsed=parseMove(move,order);if(!parsed)return null;if(parsed.halfTurn)return parsed.token.replace(/2'$/,'2');return parsed.prime?parsed.token.slice(0,-1):`${parsed.token}'`;}
  function faceSignature(faces){return FACE_ORDER.map(face=>faces[face].flat().join('')).join('|');}
  function solvedSignature(order){return faceSignature(buildState('',order).faces);}

  function selfTest(){
    const results=[],basicMoves=['U','D','F','B','R','L'];
    for(let n=2;n<=7;n++){
      const solved=solvedSignature(n),tests=[];
      for(const move of basicMoves){
        const inverse=inverseMove(move,n),state=buildState(`${move} ${inverse}`,n,{strict:true});
      ...
      if(n>=3)for(const move of ['Rw','Uw','Fw']){
        const inverse=inverseMove(move,n),state=buildState(`${move} ${inverse}`,n,{strict:true});
        tests.push({name:`${move}+inverse`,ok:faceSignature(state.faces)===solved});
      }
    ...
    if(!report.ok)console.error('[SSC Preview V1] NxN state self-test failed.',report);else console.info('[SSC Preview V1] NxN state self-test passed for 2x2-7x7.');
    return report;
  }

  window.SSCNxNState=Object.freeze({FACE_ORDER,FACE_NORMALS,normalizeScramble,parseMove,buildState,buildStickerMapping,validateFaces,stickerId,selfTest});
})();