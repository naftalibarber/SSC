(() => {
  ...
  const I18N={
    he:{
      ...
      sessionMenu:'ניהול סשנים',solves:'פתרונות',mean:'ממוצע',best:'שיא',lastActivity:'פעילות אחרונה',moreStats:'סטטיסטיקות נוספות',lessStats:'פחות סטטיסטיקות',current:'נוכחי',bestSection:'שיאים',worst:'הגרוע ביותר',solveCountLabel:'מספר פתרונות',focusMode:'מצב מיקוד',exitFocus:'יציאה ממיקוד',enterFullscreen:'מסך מלא',exitFullscreen:'יציאה ממסך מלא',fullscreenUnavailable:'מסך מלא אינו זמין בדפדפן זה',solveDeleted:'הפתרון נמחק',undo:'ביטול',details:'פרטי פתרון',finalTime:'זמן סופי',rawTime:'זמן גולמי',penalty:'עונש',date:'תאריך',event:'מקצה',session:'סשן',copyScramble:'העתק ערבוב',repeatScramble:'חזור על הערבוב',copied:'הערבוב הועתק',newPb:'PB חדש!',keyboardShortcuts:'קיצורי מקלדת',close:'סגור',shortcutNext:'ערבוב הבא',shortcutPrevious:'ערבוב קודם',shortcutFocus:'מצב מיקוד',shortcutEscape:'יציאה / סגירה',shortcutUndo:'ביטול מחיקה',shortcutOk:'סימון OK',shortcutPlus2:'סימון +2',shortcutDnf:'סימון DNF',shortcutHelp:'עזרה לקיצורים',sessionEmptyDelete:'למחוק את הסשן הריק הזה?'
    },
    en:{
  ...
  function ensureAdvancedUi(){
    if(!document.getElementById('sscTrainingStyles')){const link=document.createElement('link');link.id='sscTrainingStyles';link.rel='stylesheet';link.href='./code/css/training-upgrades.css?v=20260825-1';document.head.appendChild(link);}
    const toolbar=document.querySelector('.topbar-start');
    if(toolbar&&!document.getElementById('focusModeButton')){const button=document.createElement('button');button.id='focusModeButton';button.className='toolbar-button';button.type='button';button.innerHTML='<span>FOCUS</span>';toolbar.appendChild(button);}
    ...
    if(!document.getElementById('focusControls')){const controls=document.createElement('div');controls.id='focusControls';controls.className='focus-controls';controls.innerHTML='<button type="button" data-focus-exit></button><button type="button" data-focus-fullscreen></button>';document.body.appendChild(controls);}
  ...
  function renderHistory(){
    const history=currentSessionHistory();els.solveCount.textContent=history.length;renderStats();renderSessionMenu();els.historyList.innerHTML='';if(!history.length){els.emptyHistory.hidden=false;els.historyList.hidden=true;return;}els.emptyHistory.hidden=true;els.historyList.hidden=false;
    ...
    history.slice(0,12).forEach((solve,index)=>{const effective=getEffectiveTime(solve);const row=document.createElement('div');row.className='solve-row enhanced-solve-row'+(bestSingle!==null&&effective===bestSingle?' best-solve':'')+(solve.penalty!=='OK'?` penalty-${solve.penalty==='DNF'?'dnf':'plus2'}`:'');const date=new Date(solve.createdAt);const locale=currentLanguage==='he'?'he-IL':'en-US';const meta=`${date.toLocaleTimeString(locale,{hour:'2-digit',minute:'2-digit'})} · ${solve.scramble}`;row.innerHTML=`<div class="solve-index">${index+1}</div><div class="solve-main"><div class="solve-time">${solveDisplay(solve)}</div><div class="solve-meta"></div>${renderSolveActions(solve)}</div>`;row.querySelector('.solve-meta').textContent=meta;row.querySelectorAll('[data-penalty]').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();if(applyPenalty(solve.id,button.dataset.penalty))renderHistory();}));row.querySelector('[data-action="delete"]').addEventListener('click',event=>{event.stopPropagation();if(deleteSolve(solve.id))renderHistory();});row.addEventListener('click',event=>{if(event.target.closest('button'))return;openSolveDetails(solve.id);});els.historyList.appendChild(row);});
  }
  ...
  document.addEventListener('keydown',event=>{
    ...
    if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'){event.preventDefault();undoDelete();return;}
    if(event.ctrlKey||event.metaKey||event.altKey)return;
    const key=event.key.toLowerCase();if(key==='n'){event.preventDefault();createNewScramble();return;}if(key==='p'){event.preventDefault();document.getElementById('prevScramble')?.click();return;}if(key==='f'){event.preventDefault();setFocusMode(!isFocusMode());return;}if(event.key==='?'||(event.code==='Slash'&&event.shiftKey)){event.preventDefault();openShortcuts();return;}
    if(['1','2','3'].includes(event.key)){const latest=currentSessionHistory()[0];if(!latest)return;event.preventDefault();const penalty=event.key==='1'?'OK':event.key==='2'?'+2':'DNF';if(applyPenalty(latest.id,penalty))renderHistory();}
  });
  ...
  document.getElementById('focusModeButton').addEventListener('click',()=>setFocusMode(!isFocusMode()));document.querySelector('[data-focus-exit]').addEventListener('click',()=>setFocusMode(false));document.querySelector('[data-focus-fullscreen]').addEventListener('click',toggleFullscreen);
  ...
  els.languageToggle.addEventListener('click',()=>{currentLanguage=currentLanguage==='he'?'en':'he';localStorage.setItem(LANGUAGE_KEY,currentLanguage);applyLanguage();});els.historySettingsButton.addEventListener('click',()=>{els.historySettings.hidden=!els.historySettings.hidden;});
