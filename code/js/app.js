(() => {
  ...
  function mean(times,count){if(times.length<count)return null;const sample=times.slice(0,count);return sample.reduce((a,b)=>a+b,0)/sample.length;}
  function trimmedAverage(times,count){if(times.length<count)return null;const sample=times.slice(0,count);const sorted=[...sample].sort((a,b)=>a-b);if(count<5)return mean(times,count);const trim=Math.ceil(count*.05);const trimmed=sorted.slice(trim,sorted.length-trim);return trimmed.length?trimmed.reduce((a,b)=>a+b,0)/trimmed.length:null;}

  function normalizeSolve(solve){
    const legacyPuzzle=solve?.puzzle||'3x3';
    const fallbackEvent=eventIdForLegacyPuzzle(legacyPuzzle);
    const eventId=normalizeEventId(solve?.eventId||fallbackEvent)||fallbackEvent;
    const event=eventFor(eventId);
    return {...solve,puzzle:legacyPuzzle,eventId,puzzleId:solve?.puzzleId||event?.puzzle||legacyPuzzle,sessionId:solve?.sessionId||'session-1'};
  }
  function getHistory(){try{return(JSON.parse(localStorage.getItem(STORAGE_KEY))||[]).map(normalizeSolve);}catch{return[];}}
  ...
  function addSolve(timeMs,scramble){
    ...
    history.unshift({
      ...
      timeMs:Math.round(timeMs),scramble,puzzle:legacyPuzzleForEvent(currentEvent),eventId:currentEvent,puzzleId:event?.puzzle||currentEvent,sessionId:currentSessionId,createdAt:new Date().toISOString()
    });
    setHistory(history);
  }
  ...
  function deleteCurrentSession(){
    ...
    setHistory(getHistory().filter(solve=>!(solve.eventId===currentEvent&&solve.sessionId===currentSessionId)));
  ...
  function metricCurrent(metric,times){if(!times.length)return null;if(metric.type==='best')return times[0];if(metric.type==='mean')return mean(times,metric.count);return trimmedAverage(times,metric.count);}
  function metricBest(metric,times){if(!times.length)return null;if(metric.type==='best')return Math.min(...times);if(times.length<metric.count)return null;let best=null;for(let i=0;i<=times.length-metric.count;i++){const sample=times.slice(i,i+metric.count);const value=metric.type==='mean'?mean(sample,metric.count):trimmedAverage(sample,metric.count);if(value!==null&&(best===null||value<best))best=value;}return best;}
  ...
  function renderStats(){
    const history=currentSessionHistory();const times=history.map(solve=>solve.timeMs);
    ...
    if(els.quickAo5){const value=trimmedAverage(times,5);els.quickAo5.textContent=value===null?'—':formatTime(value);}
    if(els.quickAo12){const value=trimmedAverage(times,12);els.quickAo12.textContent=value===null?'—':formatTime(value);}
  ...
  function renderHistory(){
    ...
    els.emptyHistory.hidden=true;els.historyList.hidden=false;const bestSingle=Math.min(...history.map(solve=>solve.timeMs));
    history.slice(0,8).forEach((solve,index)=>{const row=document.createElement('div');row.className='solve-row'+(solve.timeMs===bestSingle?' best-solve':'');const date=new Date(solve.createdAt);const locale=currentLanguage==='he'?'he-IL':'en-US';const meta=`${date.toLocaleTimeString(locale,{hour:'2-digit',minute:'2-digit'})} · ${solve.scramble}`;row.innerHTML=`<div class="solve-index">${index+1}</div><div class="solve-main"><div class="solve-time">${formatTime(solve.timeMs)}</div><div class="solve-meta"></div></div><button class="delete-solve" type="button" title="${t('deleteSolve')}">×</button>`;row.querySelector('.solve-meta').textContent=meta;row.querySelector('.delete-solve').addEventListener('click',()=>{deleteSolve(solve.id);renderHistory();});els.historyList.appendChild(row);});
  }

  class TimerEngine{
    constructor({holdMs=500,onTick,onStateChange,onStop}){this.holdMs=holdMs;this.onTick=onTick;this.onStateChange=onStateChange;this.onStop=onStop;this.state='idle';this.holdTimeout=null;this.startTime=0;this.animationFrame=null;}
    setState(state){this.state=state;this.onStateChange?.(state);}
    press(){if(this.state==='running'){this.stop();return;}if(this.state!=='idle')return;this.setState('holding');this.holdTimeout=setTimeout(()=>{if(this.state==='holding')this.setState('ready');},this.holdMs);}
    release(){if(this.state==='ready')this.start();else if(this.state==='holding')this.cancelHold();}
    cancelHold(){clearTimeout(this.holdTimeout);this.holdTimeout=null;if(this.state==='holding')this.setState('idle');}
    start(){clearTimeout(this.holdTimeout);this.holdTimeout=null;this.startTime=performance.now();this.setState('running');this.tick();}
    ...
    stop(){if(this.state!=='running')return;const elapsed=performance.now()-this.startTime;cancelAnimationFrame(this.animationFrame);this.animationFrame=null;this.setState('idle');this.onStop?.(elapsed);}
  }

  function setTimerState(state){els.timer.className=state==='idle'?'':state;els.status.textContent=t(state);}
  ...
  async function setEvent(value){
    const eventId=normalizeEventId(value);if(!eventId||timer.state==='running'||eventId===currentEvent)return false;
  ...
  const timer=new TimerEngine({
    ...
    onStop:elapsed=>{els.timer.textContent=formatTime(elapsed);els.status.textContent=t('saved');addSolve(elapsed,currentScramble);renderHistory();createNewScramble();}
  });
  ...
  window.addEventListener('ssc-scramble-history-select',event=>{
    ...
    if(!scramble||eventId!==currentEvent||timer.state==='running')return;
  ...
  document.addEventListener('keydown',event=>{if(event.code!=='Space'||event.repeat)return;event.preventDefault();if(timer.state!=='running'&&!currentScramble)return;timer.press();});
  document.addEventListener('keyup',event=>{if(event.code!=='Space')return;event.preventDefault();timer.release();});
  els.touchTimer.addEventListener('pointerdown',event=>{event.preventDefault();if(timer.state!=='running'&&!currentScramble)return;els.touchTimer.setPointerCapture?.(event.pointerId);timer.press();});
  ...
  document.getElementById('clearHistory').addEventListener('click',()=>{const selected=currentSessionHistory();if(!selected.length)return;if(confirm(t('clearHistoryConfirm'))){setHistory(getHistory().filter(solve=>!(solve.eventId===currentEvent&&solve.sessionId===currentSessionId)));renderHistory();}});
  ...
  renderEventSelect();renderSessionSelect();applyLanguage();createNewScramble();
})();
