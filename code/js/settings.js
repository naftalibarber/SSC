(() => {
  ...
  const defaults={textSize:100,font:'Rubik',theme:'light',primaryColor:'#2563eb'};
  ...
  function repairSelectors(){
    if(languageSelect){
      languageSelect.innerHTML='<option value="he">עברית</option><option value="en">English</option>';
    }
    if(fontSelect){
      ...
      if(FONT_OPTIONS.some(([value])=>value===current))fontSelect.value=current;
    }
  }
  ...
  function fontStack(font){
    if(font==='Rubik')return 'Rubik,Arial,sans-serif';
    if(font==='Assistant')return 'Assistant,Arial,sans-serif';
    if(font==='Heebo')return 'Heebo,Arial,sans-serif';
    if(font==='Alef')return 'Alef,Arial,sans-serif';
    if(font==='NotoHebrew')return '"Noto Sans Hebrew",Arial,sans-serif';
  ...
  function applyAppearance(){
    ...
    const theme=allowedThemes.includes(settings.theme)?settings.theme:'light';
    ...
    textSizeRange.value=String(settings.textSize||100);
    textSizeValue.textContent=`${settings.textSize||100}%`;
    fontSelect.value=settings.font||defaults.font;
    primaryColorInput.value=settings.primaryColor;
    primaryColorValue.textContent=settings.primaryColor.toUpperCase();
  }
  ...
  function updateLabels(){
    ...
    document.getElementById('languageSettingLabel').textContent=en?'Language':'שפה';
    document.getElementById('textSizeSettingLabel').textContent=en?'Text size':'גודל הטקסט';
    document.getElementById('fontSettingLabel').textContent=en?'Font':'גופן הטקסט';
    document.getElementById('primaryColorSettingLabel').textContent=en?'Primary color':'צבע ראשי';
    document.getElementById('themeSettingLabel').textContent=en?'Theme':'עיצוב';
    document.getElementById('themeLightButton').textContent=en?'Light':'בהיר';
    document.getElementById('themeDarkButton').textContent=en?'Dark':'כהה';
    resetPrimaryColor.textContent=en?'Reset':'איפוס';
    closeButton.setAttribute('aria-label',en?'Close':'סגור');
    openButton.title=en?'General settings':'הגדרות כלליות';
    openButton.setAttribute('aria-label',en?'General settings':'הגדרות כלליות');
    languageSelect.value=en?'en':'he';
  }
  function openModal(){modal.hidden=false;updateLabels();requestAnimationFrame(()=>closeButton.focus());}
  ...
  languageSelect.addEventListener('change',()=>{
    ...
    if(target!==currentLanguage())hiddenLanguageToggle.click();
    updateLabels();
  });
